...
async def buttons_handler(event):
    ...
    if text == "➕ إضافة حساب":
        ...
        sessions = glob.glob(f"{SESSIONS_DIR}/*.session")
        if not sessions:
            await event.respond("⚠️ لا توجد حسابات مضافة! قم بإضافة حساب أولاً.")
            return

        status_msg = await event.respond("🚀 جاري بدء الفحص للحسابات المسجلة...")
        total_cleaned = 0
        ...
        await status_msg.edit(f"✅ اكتمل الفحص بنجاح!\nإجمالي المجموعات والقنوات المخالفة التي تم الخروج منها: **{total_cleaned}**")
...
async def input_handler(event):
    ...
    if state.get('step') == 'await_phone':
        ...
        try:
            ...
            await status_msg.edit("تم إرسال الكود في تطبيق تلجرام. أرسل الكود هنا:")
        except Exception as e:
        ...
        try:
            ...
            if "TWO_STEP" in str(e):
                ...
                await status_msg.edit("الحساب محمّي بكلمة مرور (التحقق بخطوتين)، أرسل كلمة المرور:")
                return
            else:
